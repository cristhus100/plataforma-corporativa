...
export default function MayorPage() {
  ...
  async function exportarExcelFn() {
    const columns = [
      { key: 'fecha', label: 'Fecha', formatter: (v, item) => item.comprobante?.fecha ? new Date(item.comprobante.fecha).toLocaleDateString('es-CO') : '' },
      { key: 'numero_comprobante', label: 'Comprobante', formatter: (v, item) => item.comprobante?.numero_comprobante || '' },
      { key: 'descripcion', label: 'Descripción', formatter: (v, item) => item.descripcion || item.comprobante?.concepto || '' },
      { key: 'debito', label: 'Débito', formatter: (v, item) => `$${Number(item.valor || 0).toLocaleString('es-CO')}` },
      { key: 'credito', label: 'Crédito', formatter: (v, item) => `$${Number(item.valor || 0).toLocaleString('es-CO')}` },
      { key: 'saldo', label: 'Saldo', formatter: (v, item, idx, allItems) => {
        let acc = 0;
        for (let i = 0; i <= idx; i++) {
          const val = Number(allItems[i].valor) || 0;
          acc += allItems[i].naturaleza === 'debito' ? val : -val;
        }
        const nat = cuentaInfo?.naturaleza || 'debito';
        const running = nat === 'debito' ? acc : -acc;
        return `$${running.toLocaleString('es-CO')}`;
      }},
    ];
    const data = (asientos || []).map((item, idx, allItems) =>
      columns.reduce((acc, col) => ({
        ...acc,
        [col.label]: col.formatter ? col.formatter(item[col.key], item, idx, allItems) : (item[col.key] ?? ''),
      }), {})
    );
    await exportarExcel(data, columns, 'libro_mayor', 'Libro Mayor - Serviequipos');
  }
