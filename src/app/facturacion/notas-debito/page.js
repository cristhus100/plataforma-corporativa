...
import { crearNotaDebito } from '@/actions/facturacion'
...
export default function NotasDebitoPage() {
  ...
  // Form state for creating new ND
    ...
    try {
      const [facturasRes, tiposRes] = await Promise.all([
        ...
        supabase.from('tipo_documentos_factura').select('id, nombre, prefijo')
          .eq('activo', true).eq('tipo', 'nota_debito').order('nombre'),
      ])
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="text-gray-600">Cargando notas d&eacute;bito...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
            ...
            <span>/</span>
            <span className="text-gray-900">Notas D&eacute;bito</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900">Notas D&eacute;bito</h1>
          <p className="text-sm text-gray-600">Documentos electr&oacute;nicos de Nota D&eacute;bito</p>
        </div>
        {isAdmin && (
          <button onClick={() => setShowForm(!showForm)}
            className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition">
            <Plus className="h-4 w-4" />
            {showForm ? 'Cancelar' : 'Nueva ND'}
          </button>
        )}
      </div>
      ...
      {showForm && (
        <div className="bg-white border border-blue-200 rounded-lg p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">Crear Nota D&eacute;bito</h2>
          {formError && (
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <input type="text" name="motivo" value={formData.motivo} onChange={handleFormChange} required
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Ej: Intereses de mora, diferencia en tasa de cambio..." />
            </div>
            <div>
            ...
            <div className="md:col-span-2 flex items-center gap-2">
              <button type="submit" disabled={formLoading}
                ...
                {formLoading ? <><Loader2 className="h-4 w-4 animate-spin" /> Creando...</> : <><Save className="h-4 w-4" /> Crear ND</>}
              </button>
            </div>
          </form>
        </div>
        ...
        {notasFiltradas.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h3 className="mt-3 text-sm font-semibold text-gray-900">
              {notas.length === 0 ? 'No hay notas d&eacute;bito registradas' : 'No se encontraron resultados'}
            </h3>
            <p className="mt-1 text-sm text-gray-500">
              {notas.length === 0 ? 'Las notas d&eacute;bito aparecer&aacute;n aqu&iacute; una vez creadas desde una factura.' : 'Intenta ajustar la b&uacute;squeda.'}
            </p>
          </div>
        ) : (
