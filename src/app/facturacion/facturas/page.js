...
export default function FacturasPage() {
  ...
  async function exportarExcelFn() {
    const columns = [
      ...
      { key: 'fecha_emision', label: 'Fecha Emisión', formatter: (v) => v ? new Date(v).toLocaleDateString('es-CO') : '' },
      { key: 'tercero_nombre', label: 'Cliente', formatter: (v, item) => item.terceros?.nombre_comercial || item.terceros?.nombre_completo || '' },
      { key: 'total', label: 'Total', formatter: (v) => v ? formatCOP(v) : '$0' },
      { key: 'estado', label: 'Estado', formatter: (v, item) => {
        const hoy = new Date()
        const vencimiento = new Date(item.fecha_vencimiento + 'T12:00:00')
        const estadoReal = item.estado === 'pendiente' && vencimiento < hoy ? 'vencida' : item.estado
        return getEstadoFacturaLabel(estadoReal)
      }},
    ]
    ...
    await exportarExcel(data, columns, 'facturas', 'Facturas - Serviequipos')
  }
