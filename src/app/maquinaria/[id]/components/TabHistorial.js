...
export default function TabHistorial({ maquinariaId, maquinaria, isAdmin = false }) {
  ...
  const [formPrev, setFormPrev] = useState({ titulo: '', descripcion: '', fecha: '', horometro: '', operador_nombre: '', costo: '', proveedor: '' });
  ...
  const [formCorr, setFormCorr] = useState({ titulo: '', descripcion: '', fecha: '', horometro: '', operador_nombre: '', costo: '', proveedor: '' });
  ...
  const cargarPreventivos = useCallback(async () => {
    try {
      ...
      const [resMant, resReg] = await Promise.all([
        supabase.from('mantenimientos').select('*').eq('maquinaria_id', maquinariaId).eq('tipo', 'preventivo').order('fecha', { ascending: false }),
        supabase.from('registros_horometro').select('*').eq('maquinaria_id', maquinariaId).eq('es_cambio_aceite', true).order('fecha', { ascending: false }),
      ]);
      ...
      const cambiosAceite = (resReg.data || []).map(r => ({
        ...
        descripcion: `Realizado a las ${r.horometro_final} hrs`,
        fecha: r.fecha,
        horometro: r.horometro_final || r.horometro_inicial,
  ...
  const guardarMantenimiento = async (tipo, form, setForm, setSaving, setShowForm, cargarFn) => {
    ...
    try {
      ...
      setForm({ titulo: '', descripcion: '', fecha: hoyStr, horometro: '', operador_nombre: '', costo: '', proveedor: '' });
      setShowForm(false);
  ...
  const renderFormulario = (form, setForm, saving, onGuardar, onCancelar) => (
    <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 mb-4 space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Horómetro</label>
          <input type="number" name="horometro" value={form.horometro} onChange={(e) => handleFormChange(e, setForm)} placeholder={maquinaria.horometro_actual || '0'} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" />
        </div>
        <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-right py-2 px-3 font-semibold text-gray-600">Horómetro</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {preventivos.map((item) => (
                <tr key={item.id} className={`hover:bg-gray-50 ${item.es_cambio_aceite ? 'bg-yellow-50/50' : ''}`}>
                  ...
                  <td className="py-2 px-3 text-right font-mono">{item.horometro ? `${item.horometro} hrs` : '—'}</td>
            ...
            <tbody className="divide-y divide-gray-100">
              {tanqueos.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-2 px-3 text-right font-mono">{item.horometro_final || item.horometro_inicial} hrs</td>
